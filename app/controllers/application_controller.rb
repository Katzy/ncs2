class ApplicationController < ActionController::Base
 ...
 # before_action :configure_devise_permitted_parameters, if: :devise_controller?
  ...
  def initialize_users_for_header
    ...
    @user = current_user
    @schools = School.where('league_id IS NOT null').order('name ASC')
    count = @schools.count
    half = count / 2
  ...
  def authorize_user
    ...
    redirect_to root_path, alert: 'Access Denied' unless (user_signed_in? && (current_user.admin? || (current_user.league_rep == true) || (current_user.coach == true)))
  end
